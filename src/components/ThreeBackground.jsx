...
// Helper to create a cloud group (same as mockup)
...
// Helper to create chairs, umbrella, silhouettes (we'll copy from mockup)
...
function createUmbrella(x, z) {
  ...
  const canopy = new THREE.Mesh(
    new THREE.ConeGeometry(1.6, 0.7, 8),
...
function createYacht() {
  ...
  const emblem = new THREE.Mesh(
    ...
    new THREE.MeshStandardMaterial({ color: 0xc9a96e, emissive: 0xc9a96e, emissiveIntensity: 0.2 })
  );
...
export default function ThreeBackground({ sceneIndex, transitionProgress }) {
  ...
  // Positions for each scene
  const targetPositions = [
    { x: 0, y: 2, z: 10, target: [0, 0, 0] }, // Home
    { x: 0, y: 3.5, z: 8, target: [0, 2, -3] }, // About
    { x: 0, y: 3.5, z: 14, target: [0, -0.5, -8] }, // Services
    { x: 2, y: 2.5, z: 6, target: [0, 0.5, -10] }, // Contact
  ];
  const bgColors = [
    0x0b0a08, // space
    0x4a90d9, // sky
  ...
  useEffect(() => {
    ...
    // ---- Build scene groups (same as mockup) ----
    // Space
    ...
    for (let i = 0; i < 40; i++) {
      ...
      const cloud = createCloud(x, y, z, scale, 0xffeedd, 0xffaa55);
    ...
    // Initially show space
    spaceGroup.visible = true;
    aboutGroup.visible = false;
    beachGroup.visible = false;
    contactGroup.visible = false;

    // ---- Logo (optional – we'll render it in React instead) ----
    // We'll skip the logo here, we can add it as a React element on top.
    ...
    const animate = () => {
      ...
      // Animate clouds in about and contact
      ...
      // Ocean waves (simplified: we'll just animate the geometry if we have refs)
  ...
  useEffect(() => {
    ...
    const fromIndex = Math.floor(sceneIndex); // we'll handle transition separately
    ...
    const blended = bg1.clone().lerp(bg2, ease);
    ...
    if (t < 0.5) {
      groups.forEach((g, i) => {
        if (g) g.visible = i === fromIndex;
      });
    } else {
      groups.forEach((g, i) => {
        if (g) g.visible = i === toIndex;
      });
    }
